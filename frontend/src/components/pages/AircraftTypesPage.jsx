...
const AircraftTypesPage = () => {
  ...
  // Map for valid aircraft classes
  const classMap = ["light", "midsize", "supermidsize", "heavy", "ultralong"];

  // Determine initial tab index based on URL parameter
  const getInitialTabIndex = () => {
    if (!type) return 0;
    const index = classMap.indexOf(type);
  ...
  // Current class based on tabIndex
  const currentClass = classMap[tabIndex];

  // React to URL parameter changes
  useEffect(() => {
    if (type) {
      const typeIndex = classMap.indexOf(type);
      if (typeIndex >= 0 && typeIndex !== tabIndex) {
        setTabIndex(typeIndex);
      }
    }
  }, [type, classMap, tabIndex]);
      ...
      onSuccess: (data) => {
        ...
        if (isRefreshing) {
          toast({
            ...
            description:
              "Aircraft data has been updated with the latest information",
            status: "success",
            duration: 3000,
            isClosable: true,
          });
  ...
  const handleTabChange = (index) => {
    ...
    const newType = classMap[index];
    navigate(`/aircraft-types/${newType}`, { replace: true });
  };
  ...
  const viewAircraft = (aircraft) => {
    ...
    setTimeout(() => {
      document
        .getElementById("aircraft-details")
  ...
  const filterAircraftWithImages = (aircraftData) => {
    ...
    return {
      ...aircraftData,
      data: aircraftData.data.filter(
        (aircraft) => aircraft.images && aircraft.images.length > 0
      ),
    };
  };
  ...
  const processAircraftForDisplay = (aircraft) => {
    const mainImageUrl =
      aircraft.images && aircraft.images.length > 0
        ? aircraft.images.find((img) => img.tag === "exterior")?.path ||
          aircraft.images[0].path
        : `/images/aircraft/placeholders/${aircraft.aircraftType.name
            .toLowerCase()
            .replace(/\s+/g, "-")}.jpg`;
  ...
  const getDefaultDescription = (aircraft) => {
    ...
    return `The ${type} is a versatile ${className.toLowerCase()} jet offering exceptional performance and comfort for ${
      className.toLowerCase() === "light"
        ? "short to medium"
        : className.toLowerCase() === "ultralong"
        ? "ultra-long"
        : "medium to long"
    }-range flights. With its ${
      className.toLowerCase() === "light" ? "efficient" : "spacious"
    } cabin and advanced avionics, it provides an efficient and comfortable travel experience for up to ${
      aircraft.passengersMax
    } passengers.`;
  };
    ...
    <Container maxW="container.xl" py={10}>
      ...
      <Box textAlign="center" mb={20}>
        ...
        <Text fontSize="xl" maxW="3xl" mx="auto" color="gray.600">
          Explore our diverse fleet of private jets to find the perfect aircraft
                ...
                <Box>
                  ...
                  {isLoading ? (
                    ...
                    <Alert status="error" borderRadius="md">
                      <AlertIcon />
                      {error?.message ||
                        "Failed to load aircraft data. Please try again later."}
                    </Alert>
                      ...
                      <SimpleGrid columns={{ base: 1, lg: 2 }} spacing={8}>
                        {filteredData.data.map((aircraft) => {
                          const displayAircraft =
                            processAircraftForDisplay(aircraft);
                          return (
                            <AircraftCard
                    ...
                    <Box textAlign="center" py={10}>
                      <Text fontSize="lg">
                        No aircraft found with images for this category.
                      </Text>
                    </Box>
                  )}
                </Box>
          ...
          <Grid templateColumns={{ base: "1fr", lg: "1fr 1fr" }} gap={8}>
            <GridItem>
              {selectedAircraft.images && selectedAircraft.images.length > 0 ? (
                <Image
                  src={
                    selectedAircraft.images.find(
                      (img) => img.tag === "exterior"
                    )?.path || selectedAircraft.images[0].path
                  }
                  ...
                  height="auto"
                  objectFit="cover"
                  fallbackSrc={`/images/aircraft/placeholders/${selectedAircraft.aircraftType.name
                    .toLowerCase()
                    .replace(/\s+/g, "-")}.jpg`}
                />
              ) : (
                <Image
                  src={`/images/aircraft/placeholders/${selectedAircraft.aircraftType.name
                    .toLowerCase()
                    .replace(/\s+/g, "-")}.jpg`}
                  alt={selectedAircraft.aircraftType.name}
                  borderRadius="lg"
              ...
              <VStack align="start" spacing={6}>
                <Text fontSize="lg">
                  {selectedAircraft.description ||
                    getDefaultDescription(selectedAircraft)}
                </Text>
                ...
                <SimpleGrid columns={2} spacing={6} width="100%">
                  ...
                  <Specification
                    ...
                    title="Luggage"
                    value={`${
                      selectedAircraft.features?.luggageVolume || "N/A"
                    } cubic meters`}
                  />
                </SimpleGrid>
...
const getAmenities = (aircraft) => {
  ...
  if (aircraft.features.entertainmentSystem)
    amenities.push("Entertainment System");
